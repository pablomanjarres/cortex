  ...
  useEffect(() => {
    if (window.electronAPI?.tray) {
      window.electronAPI.tray.updateStats({
        ...
        habits: `${habitsCompleted}/${activeHabits.length}`,
        score: '',
      })
    }
  }, [sessionCount, habitsCompleted, activeHabits.length])
  ...
  return (
    <PageShell>
      ...
      <div className="grid grid-cols-1 gap-5 lg:grid-cols-2">
        ...
        {/* Compact Habits */}
        <WidgetCard title="Habits" description={`${habitsCompleted}/${activeHabits.length}`} delay={0.2} compact>
          <div className="flex items-center justify-between">
            {activeHabits.map((h) => (
              <Button
                key={h.id}
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        />
        <StatTile label="Habits" value={`${habitsCompleted}/${activeHabits.length}`} />
      </div>
    </PageShell>
  )
}
