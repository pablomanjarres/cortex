...
export function SprintProvider({ children }: { children: ReactNode }) {
  // Reactive day key — rolls over at midnight so the sessions store re-keys
  // to the new day instead of silently writing into yesterday's list.
  const today = useToday()
  const [sessions] = useStore<SprintSession[]>(`cortex-daily-sessions-${today}`, [])

  // Log a completed session to the day the sprint STARTED (derived from its
  // start timestamp at completion time). A sprint that starts before midnight
  // and completes after belongs to the day it began. This deliberately does a
  // read-modify-write against the persisted store instead of going through the
  // useStore updater: the updater is bound to a day key captured in effect
  // closures, and after a rollover a stale updater would clobber the old day's
  // list with the new day's state. The UI picks the write up via the store's
  // 2s sync poll.
  const logSession = useCallback((session: SprintSession) => {
    const dayKey = localDate(new Date(session.startedAt))
    const storeKey = `cortex-daily-sessions-${dayKey}`
    readStore<SprintSession[]>(storeKey, []).then((prev) => {
      writeStore(storeKey, [...prev, session])
    })
  }, [])
  ...
  useEffect(() => {
    ...
    readStore<PersistedSprint | null>('cortex-active-sprint', null).then((saved) => {
      ...
      if (saved.isPaused) {
        ...
        if (remaining <= 0) {
          // Completed while app was closed — save session (to its start day)
          logSession({
